...
11. SANDBOX FIRST — NEVER write Python scripts, shell scripts, or suggest manual execution. Use evaluate_expression for TypeScript in the sandbox. npm packages are loaded from esm.sh as browser ESM bundles. BEFORE writing sandbox code for binary file generation (PPTX, XLSX, PDF, images): read the sandbox-environment skill for proven patterns and constraints. KEY CONSTRAINTS: No Blob, No Buffer, No DOM, No require(), No fetch(). Binary output MUST use ArrayBuffer/Uint8Array (outputType:"arraybuffer" or writeBuffer()). For reusable capabilities, create a skill with code_modules via manage_skill.`;
}
